...
class HuggingFaceProvider(DataProvider):
    ...
    def prepare_local_data(
        ...
        **kwargs,
        ...
        logger.info(
            f"Downloading HuggingFace dataset '{self.dataset_name}' to {target_dir}"
        )
        ...
        try:
            ...
            logger.info(
                f"Downloaded HuggingFace dataset '{self.dataset_name}' to {target_dir}"
            )
        except ValueError as e:
            if "Invalid pattern" in str(e) and "**" in str(e):
                # Fallback to snapshot_download for Git repositories
                logger.info(
                    "Dataset appears to be a Git repository, using snapshot_download fallback"
                )
                try:
                ...
                logger.info(
                    f"Downloaded HuggingFace repository '{self.dataset_name}' to {target_dir}"
                )
            else:
                raise  # Re-raise if it's a different ValueError

    def prepare_modal_data(
        self, task_id: str, dataset_kwargs: dict | None = None, **kwargs
    ) -> str:
        """Download directly to Modal volume."""
        from ..runtime.modal_data_functions import (
        ...
        try:
            ...
            if task_contents:  # Directory exists and has contents
                logger.info(
                    f"Dataset already exists for task {task_id}, skipping download"
                )
                return f"Dataset {self.dataset_name} already exists at tasks/{task_id}"
        except Exception:
            # Directory doesn't exist, proceed with download
            logger.info(
                f"Downloading HuggingFace dataset '{self.dataset_name}' to Modal volume for task {task_id}"
            )
            with app.run():
                result = download_huggingface_data.remote(
                    self.dataset_name, task_id, dataset_kwargs
                )
