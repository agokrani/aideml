...
from aide.actions.action import Debug, Draft, Improve
...
class CoPilot(Workflow):
    ...
    async def run(self):
        ...
        if len(self.journal) == 0:
            messages.append({"role": "assistant", "content": self.agent._initial_research})
            await self.callback_manager.execute_callback("tool_output", self.agent._initial_research)
            ...
            if best_node is not None:
                ...
                await self.callback_manager.execute_callback("tool_output", best_solution_message)

                message = f"Would you like to improve this solution? or draft a new one?"
                messages.append({"role": "assistant", "content": message})
                await self.callback_manager.execute_callback("tool_output", message)
                current_node = best_node
            else:
                assert len(self.journal) == 1, "Please specify node_id of the initial solution in the config"
        ...
        while True:
            ...
            exec_result = await self.callback_manager.execute_callback("exec", current_node.code, True)
            ...
            tool_response_func = provider_to_tool_response_message_func[next_action.tool_call_metadata["provider"]]
            
            messages.append(tool_response_func(next_action.tool_call_metadata["tool_call_id"], message))
            ...
            if best_node is not None:
                if best_node.id == current_node.id:
                    ...
                    await self.callback_manager.execute_callback("tool_output", f"Node {current_node.id} is the best node so far")
    ...
    async def step(self, action: BaseModel, parent_node:Node|None = None): 
        ...
        await self.callback_manager.execute_callback("tool_output", f"plan:\n{result_node.plan}\n\n")
        await self.callback_manager.execute_callback("tool_output", f"code:\n{result_node.code}\n\n")
