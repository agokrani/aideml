...
import pytest
from aide.journal import Journal, Node, InteractiveSession, get_path_to_node, get_longest_path, filter_on_path, filter_for_best_path, filter_for_longest_path, filter_journal, cache_best_node
...
def test_node_debug_depth(sample_nodes):
    ...
    assert n1.debug_depth == 0, "Root node should have debug_depth 0 as it's a draft node"
...
def test_filter_on_path(journal, sample_nodes):
    ...
    path = [sample_nodes[0].id, sample_nodes[1].id, sample_nodes[3].id]  # n1 -> n2 -> n4
