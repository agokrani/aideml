...
@pytest.mark.asyncio
async def test_callback_manager_execute_callback_with_error(
    callback_manager, mock_logger
):
    mock_callback = MagicMock()
...
async def test_execute_code_async(mock_live, mock_spinner):
    ...
    mock_spinner.assert_called_once_with(
        "dots", text="[magenta]Executing code...[/magenta]"
    )
    ...
    mock_live_context.update.assert_called_once_with(
        "[bold red]Done Executing the code[/bold red]"
    )
